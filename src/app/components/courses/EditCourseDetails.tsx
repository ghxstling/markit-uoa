    ...
    async function handleSubmit() {
        ...
        try {
            ...
            if (response.status !== 200) {
                // Assuming 200 is the status code for success
                // Handle server-side error
                console.error('Error:', data.statusText)
                setSnackbarMessage(data.statusText || 'Failed to edit course.')
            ...
            setSnackbarMessage('Course successfully edited!')
            setSnackbarSeverity('success')
            setOpenSnackbar(true)
        } catch (error) {
            // Handle network or other unknown errors
            console.error('Error:', error)
            setSnackbarMessage('Failed to edit course.')
            setSnackbarSeverity('error')
            setOpenSnackbar(true)
        }
    }
    ...
    return (
        <Container maxWidth="sm" style={{ marginTop: '2em' }}>
            <Paper elevation={3} style={{ padding: '2em' }}>
                ...
                >
                    Edit Course Details for Id {params.courseId}
                </Typography>
